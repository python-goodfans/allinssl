...
export default defineComponent({
	name: 'RegisterView',
	setup() {
		const { loading, error, handleSubmit, handleKeyup, registerData, handleGetCode, codeImg, formRef } =
			useController()
					...
					<div class={styles.loginBox}>
						<div class={styles.leftSection}>
							...
							<div class={styles.leftImageWrapper}>
								<img src="/static/images/login-display.svg" alt="register" class={styles.leftImage} />
							</div>
						</div>
						<div class={styles.rightSection}>
							<div class={styles.formContainer}>
								<h1 class={styles.title}>注册账号</h1>
								<NForm ref={formRef} model={registerData.value} onSubmit={handleSubmit} class={styles.formWrapper}>
									<div class={styles.formContent}>
										<div class={styles.formInputs}>
											...
											>
												<NInput
													v-model:value={registerData.value.username}
													onKeyup={handleKeyup}
											...
											<NFormItem
												show-label={false}
												path="email"
												rule={{ required: true, message: '请输入邮箱', trigger: ['input', 'blur'] }}
											>
												<NInput
													v-model:value={registerData.value.email}
													onKeyup={handleKeyup}
													disabled={loading.value}
													placeholder="请输入邮箱"
													clearable
											...
											>
												<NInput
													onKeyup={handleKeyup}
													disabled={loading.value}
													v-model:value={registerData.value.password}
													type="password"
													placeholder="请输入密码"
													clearable
													size="large"
											...
											>
												<NInput
													onKeyup={handleKeyup}
													disabled={loading.value}
													v-model:value={registerData.value.confirmPassword}
													type="password"
													placeholder="请再次输入密码"
													clearable
													size="large"
											...
											<NFormItem
												show-label={false}
												path="code"
												rule={{ required: true, message: '请输入验证码', trigger: ['input', 'blur'] }}
											>
												<NInput
													onKeyup={handleKeyup}
													disabled={loading.value}
													v-model:value={registerData.value.code}
													type="text"
													placeholder="请输入验证码"
													clearable
													size="large"
